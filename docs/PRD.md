...
| **M6** | Vertical slice = **LitD game v0.1** | Playable skirmish vs the real AI domain: build, train, fight, win/lose; **the flagship game, not a tech demo (D-24)** — art style/factions/lore established; **mid-game save/load shipping (D-9)**; **world archive format defined (D-14)**; all §5.3 budgets green in CI; **replay verification (G5.3) green — lockstep-readiness proof for M7** |
