...
5. **`ExecuteFunc(name string)`** (call-by-string-name) is tombstoned — no reflection-based dispatch; Go closures cover the capability. Record loudly: it's the one "dynamic" JASS feature deliberately not ported (the v1/M5 Lua embedding restores it for runtime worlds — Lua calls functions by name natively; *revised 2026-06-11 per D-2026-06-11-8*).
