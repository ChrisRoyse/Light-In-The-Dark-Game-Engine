...
// #311 settings-screen runtime builder FSV (headless, no GL). SoT = the
// settingsRuntimeDump for each scenario: known settings => known resolved value
// rows, formatted text, empty Issues.
func TestBuildSettingsRuntimeFSV(t *testing.T) {
	defer chdirRepoRoot(t)()
	canvas, err := lithud.NewCanvas(1280, 960, 1)
	if err != nil {
		t.Fatal(err)
	}
	table := mustRenderDemoLocale(t)
	...
	cases := []struct {
		...
		{"default", map[string]string{
			lithud.SettingsRowName(0): "Graphics: High",
			lithud.SettingsRowName(1): "Master: 1.0",
			lithud.SettingsRowName(7): "Hotkeys: Grid",
			lithud.SettingsBackName:   "Back",
		}},
		{"custom", map[string]string{
			lithud.SettingsRowName(0): "Graphics: Low",
			lithud.SettingsRowName(1): "Master: 0.8",
			lithud.SettingsRowName(3): "Interface: 0.0",
			lithud.SettingsRowName(7): "Hotkeys: Classic",
		}},
	}
	for _, c := range cases {
		dump, err := buildSettingsRuntime(canvas, "en", table, c.scenario, 2)
		if err != nil {
			t.Fatalf("%s: %v", c.scenario, err)
		}
		t.Logf("FSV renderdemo settings scenario=%s ok=%v settings=%+v errors=%v", c.scenario, dump.OK, dump.Settings, dump.Errors)
		if !dump.OK || len(dump.Layout.Issues) != 0 {
			t.Fatalf("%s dump not OK: %+v", c.scenario, dump)
		}
		for name, wantText := range c.want {
			got := ""
			for _, l := range dump.Layout.Labels {
				if l.Name == name {
					got = l.Text
				}
			}
			if got != wantText {
				t.Fatalf("%s row %s = %q, want %q", c.scenario, name, got, wantText)
			}
		}
		// Focus row 2 must be the highlighted one.
		for _, l := range dump.Layout.Labels {
			if l.Name == lithud.SettingsRowName(2) && !l.Focused {
				t.Fatalf("%s: focused row 2 not highlighted", c.scenario)
			}
		}
	}

	// Edge: an unknown scenario errors cleanly (fail-closed), never a blank screen.
	if _, err := buildSettingsRuntime(canvas, "en", table, "bogus", 0); err == nil {
		t.Fatal("unknown settings scenario must error")
	}
}

// #71 stall-overlay runtime builder FSV (headless, no GL). SoT = the
// stallRuntimeDump: known scenario => known laggard rows + countdown.
func TestBuildStallRuntimeFSV(t *testing.T) {
	defer chdirRepoRoot(t)()
	canvas, err := lithud.NewCanvas(1280, 960, 1)
	if err != nil {
		t.Fatal(err)
	...
	cases := []struct {
		...
		{"two", map[string]string{
			...
			lithud.StallRowName(0):    "Waiting for Ser Caldus",
			lithud.StallRowName(1):    "Waiting for Mira Vale",
			lithud.StallCountdownName: "Dropping in 17s",
		}},
		{"imminent", map[string]string{
			lithud.StallRowName(0):    "Waiting for Mira Vale",
			lithud.StallCountdownName: "Dropping in 0s",
		}},
	}
	for _, c := range cases {
		dump, err := buildStallRuntime(canvas, "en", table, c.scenario)
		if err != nil {
			t.Fatalf("%s: %v", c.scenario, err)
		}
		t.Logf("FSV renderdemo stall scenario=%s ok=%v labels=%d", c.scenario, dump.OK, len(dump.Layout.Labels))
		if !dump.OK || len(dump.Layout.Issues) != 0 {
			t.Fatalf("%s dump not OK: %+v", c.scenario, dump)
		}
		for name, wantText := range c.want {
	...
	// Edge: unknown scenario errors (fail-closed).
	if _, err := buildStallRuntime(canvas, "en", table, "bogus"); err == nil {
		t.Fatal("unknown stall scenario must error")
	}
}

// #80 lobby-scene runtime builder FSV (headless, no GL). SoT = the lobbyRuntimeDump:
// known scenario => known slot rows + Start-row text + canStart.
func TestBuildLobbyRuntimeFSV(t *testing.T) {
	defer chdirRepoRoot(t)()
	canvas, err := lithud.NewCanvas(1280, 960, 1)
	if err != nil {
		t.Fatal(err)
	...
	cases := []struct {
		...
		{"mixed", false, map[string]string{
			lithud.LobbyRowName(0): "1. Aldric — Host",
			lithud.LobbyRowName(1): "2. Bryn — Ready",
			lithud.LobbyRowName(2): "3. Cael — Waiting",
			lithud.LobbyRowName(3): "4. Open",
			lithud.LobbyStartName:  "Start Game (waiting…)",
		}},
		{"ready", true, map[string]string{
			lithud.LobbyRowName(2): "3. Cael — Ready",
			lithud.LobbyStartName:  "Start Game",
		}},
	}
	for _, c := range cases {
		dump, err := buildLobbyRuntime(canvas, "en", table, c.scenario)
		if err != nil {
			t.Fatalf("%s: %v", c.scenario, err)
		}
		t.Logf("FSV renderdemo lobby scenario=%s ok=%v canStart=%v labels=%d", c.scenario, dump.OK, dump.Layout.CanStart, len(dump.Layout.Labels))
		if !dump.OK || len(dump.Layout.Issues) != 0 {
			t.Fatalf("%s dump not OK: %+v", c.scenario, dump)
		}
		if dump.Layout.CanStart != c.wantCanStart {
			t.Fatalf("%s canStart=%v, want %v", c.scenario, dump.Layout.CanStart, c.wantCanStart)
		}
		for name, wantText := range c.want {
			got := ""
			for _, l := range dump.Layout.Labels {
	...
	// Edge: unknown scenario errors (fail-closed).
	if _, err := buildLobbyRuntime(canvas, "en", table, "bogus"); err == nil {
		t.Fatal("unknown lobby scenario must error")
	}
}
